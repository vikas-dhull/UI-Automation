...
public class POS_Test_Place_Order_Offer_Split_Payment extends ExtentReportingBaseUtil
{
	private String gatewayOrderId=null;
	
	@Test(priority=1,enabled=true)
	public void testOrderPlacePOSSplitPayment() {
		comnFunc.reportLogAndPrintInConsole("## POS ORDER PLACE TEST WITH SPLIT PAYMENT APPLYING COUPON OFFER + HKCASH STARTS..");
		...
		Assert.assertTrue(posSteps.performApplyAndVerifyCouponOffer(testdata.get("POS_Coupon_Code")), "NOT able to apply Coupon Offer on POS order..!!");
		comnFunc.reportLogAndPrintInConsole("Coupon Offer applied successfully for POS order..");
		...
		Assert.assertTrue(posSteps.performPayments("Split Payment"),"Payment was not successfull..!!");
