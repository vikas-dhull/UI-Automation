...
public class HK_TestBXGYOrderPlaceCODLoginPopup extends ExtentReportingBaseUtil{
	
	private String gatewayOrderId=null;
	private int cartCount;
	/**
	 * Test Order placement COD with BXGY Offer.
	 */
	@Test(priority = 1)
	public void testBxGyOrderPlaceCODWithLoginPopup() {	
		...
		Assert.assertTrue(pdpB.verifyPDPPageLoaded(), "Fail to load BxGy PDP page..");
		test.log(LogStatus.INFO, "Product details page loaded for BXGY offer..");
		
		pdpB.addVariantToCart();
		System.out.println("verifying cart page");
		Assert.assertTrue(cartB.verifyCartPageLoaded(), "Fail to load Cart page..");
		test.log(LogStatus.INFO, "Product added to cart..");
		Assert.assertTrue(cartB.verifyBxGyOfferApplied(), "Fail to Apply BXGY Offer on Cart page..");
		...
		test.log(LogStatus.INFO, "CANCEL COD ORDERORDER(BXGY OFFER) Test starts..");		
		homeB.openMyProfile();
