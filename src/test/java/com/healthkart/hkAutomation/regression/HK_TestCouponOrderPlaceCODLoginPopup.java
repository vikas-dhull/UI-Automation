...
public class HK_TestCouponOrderPlaceCODLoginPopup extends ExtentReportingBaseUtil{
	
	private String gatewayOrderId=null;
	private int cartCount;
	
	/**
	 * Test Order placement COD with Coupon Offer.
	 */
	@Test(priority = 1)
	public void testCouponOrderPlaceCODWithLoginPopup() {	
		System.out.println("## ORDER PLACE COD (COUPON OFFER) LOGIN FROM POPUP TEST STARTS..");
		test.log(LogStatus.INFO, "ORDER PLACE COD(COUPON OFFER) LOGIN FROM POPUP TEST STARTS..");
		/*
		...
		Assert.assertTrue(pdpB.verifyPDPPageLoaded(), "Fail to load Coupon Offer PDP page..");
		test.log(LogStatus.INFO, "Product details page loaded for Coupon offer..");	
		pdpB.addVariantToCart();
		System.out.println("verifying cart page");
		Assert.assertTrue(cartB.verifyCartPageLoaded(), "Fail to load Cart page..");
		test.log(LogStatus.INFO, "Product added to cart..");
		Assert.assertTrue(cartB.applyCouponCodeAndVerify(testdata.get("Coupon_Code")), "Fail to Apply coupon Offer on Cart page..");		
		System.out.println("cart page verified");
		test.log(LogStatus.INFO, "Coupon offer applied and discount verified on cart page..");		
		...
		System.out.println("## CANCEL COD ORDER (COUPON OFFER) Test starts..");
		test.log(LogStatus.INFO, "CANCEL COD ORDER(COUPON OFFER) Test starts..");		
		homeB.openMyProfile();
	...
	@Test(dependsOnMethods=("testCouponOrderPlaceCODWithLoginPopup"))
	public void cancelCodCouponOrder() {
		System.out.println("## CANCEL COD ORDER (COUPON OFFER) Test starts..");
		test.log(LogStatus.INFO, "CANCEL COD ORDER(COUPON OFFER) Test starts..");		
		homeB.performLoginWithMobilePassword(testdata.get("Login_User_Mobile"), testdata.get("Login_User_Mobile_Password"));
		Assert.assertTrue(homeB.verifyUserLogin(), "User failed to Login");
