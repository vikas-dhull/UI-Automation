...
public class HK_TestOrderPlaceCreditCardLoginPopup extends ExtentReportingBaseUtil {
	
	private String gatewayOrderId=null;
	private int cartCount;
	
	/**
	 * Test Order Place with Credit Card.
	 */
	@Test(priority = 1)
	public void testOrderPlaceCreditCardLoginPopup() {	
		System.out.println("## ORDER PLACE PREPAID WITH CREDIT CARD Test starts..");
		test.log(LogStatus.INFO, "ORDER PLACE PREPAID WITH CREDIT CARD Test starts..");
		/*
		...
		if(!(GlobalVar.jenkinsEnvironment==null && "prod".equalsIgnoreCase(PropertyHelper.readProperty("env")) || "prod".equalsIgnoreCase(GlobalVar.jenkinsEnvironment)))
		{
			...
			System.out.println("## CANCEL PREPAID ORDER (CREDIT CARD) Test starts..");
			test.log(LogStatus.INFO, "CANCEL PREPAID ORDER (CREDIT CARD) Test starts..");
			homeB.openMyProfile();
	...
	@Test(dependsOnMethods=( "testOrderPlaceCreditCardLoginPopup"))
